...
SYMBOLIC_LIB = {'x': (lambda x: x, lambda x: x),
                 ...
                 'x^22': (lambda x: x**2.2, lambda x: x**2.2),
...
def create_dataset(f,
    ...
    # 加入高斯噪声
    if noise_std > 0:
        train_noise = torch.randn(train_label.shape) * noise_std
        test_noise = torch.randn(test_label.shape) * noise_std
        train_label += train_noise
        test_label += test_noise
