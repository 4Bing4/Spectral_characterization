...
class KAN(nn.Module):
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        for _ in pbar:
            ...
            # 更改：输出预测数据
            # train_pred = self.forward(dataset['train_input'][train_id].to(device))
            # tst_pred = self.forward(dataset['test_input'][test_id].to(device))
            # train_pred = dataset['train_label'].to(device)
            # np.savetxt('xyzpre.txt', tst_pred.detach().cpu().numpy())
            # np.savetxt('trainpre.txt', train_pred.detach().cpu().numpy())
