...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device='cpu'):
        ...
        if isinstance(scale_base, float):
            ...
            self.scale_base = torch.nn.Parameter(torch.DoubleTensor(scale_base).cuda()).requires_grad_(sb_trainable)
    ...
    def forward(self, x):
        ...
        # # 计算 x0=1 对应的 y0
        # x0 = torch.tensor([0.0], device=self.device).unsqueeze(0)  # shape: (1,)
        # # preacts0 = x0.permute(1, 0).clone().reshape(batch, self.out_dim, self.in_dim)
        # base0 = self.base_fun(x0).permute(1, 0)  # shape (batch, size)
        # y0 = coef2curve(x_eval=x0, grid=self.grid[self.weight_sharing], coef=self.coef[self.weight_sharing], k=self.k,
        #                 device=self.device)
        # # 计算 x0=1 对应的 y0
        # x1 = torch.tensor([1.0], device=self.device).unsqueeze(0)  # shape: (1,)
        # # preacts0 = x0.permute(1, 0).clone().reshape(batch, self.out_dim, self.in_dim)
        # base1 = self.base_fun(x1).permute(1, 0)  # shape (batch, size)
        # y1 = coef2curve(x_eval=x1, grid=self.grid[self.weight_sharing], coef=self.coef[self.weight_sharing], k=self.k,
        #                 device=self.device)

        y = self.scale_base.unsqueeze(dim=0) * base + self.scale_sp.unsqueeze(dim=0) * y
        # y0 = self.scale_base.unsqueeze(dim=0) * base0 + self.scale_sp.unsqueeze(dim=0) * y0
        # y1 = self.scale_base.unsqueeze(dim=0) * base1 + self.scale_sp.unsqueeze(dim=0) * y1

        # 使用 y0 对 y 进行归一化
        # y = y/y0
        # y = (y-y0) / (y1-y0)  # shape: (batch, size)
